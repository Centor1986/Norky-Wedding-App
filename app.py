...
def init_database():
    ...
    VALUES
    (1, 'Ashley & Matthew', '', 'Deannie Landgoed', 'Welcome to our wedding experience.', '#', '#', '#', 0)
    """)
...
def couple_login():
    ...
    return render_template("couple_login.html")


@app.route("/couple-logout")
def couple_logout():
    session.pop("couple_logged_in", None)
...
def manage_weddings():
    ...
    if request.method == "POST":
        ...
        FROM weddings
        WHERE wedding_date = ?
        """, (wedding_date,)).fetchone()

        if existing:
            connection.close()
            flash("This wedding date is already booked.")
            return redirect("/admin/weddings")

        connection.execute("""
        INSERT INTO weddings
        (couple_names, wedding_date, venue_name, couple_password)
        VALUES (?, ?, ?, ?)
        """, (couple_names, wedding_date, venue_name, couple_password))
        ...
        flash("Wedding added successfully.")
        return redirect("/admin/weddings")
    ...
    return render_template("manage_weddings.html", weddings=weddings)
